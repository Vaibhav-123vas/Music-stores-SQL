...
/* METHOD-2  First calculate total sales for all artists and then find the best selling total sales for all artists  Which will be ordered in descending order by total_sales*/
...
/* Method : first calculate the total purchases for each country  and for each  genre  then find the maximum purchase done taken from CTE 2 and then join the 2 CTES */
...
JOIN 
    max_spending ms ON cs.country = ms.country AND cs.total_spent = ms.max_spent
ORDER BY 
